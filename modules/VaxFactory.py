...
class VaxLocation:
	...
	def pin(self, google = True, apple = True, bing = True):
		pins = []
		if google:
			pins.append("https://www.google.com/maps/search/?api=1&query=" + str(self.lati) + "," + str(self.longi))
		if apple:
			pins.append("http://maps.apple.com/?sll=" + str(self.lati) + "," + str(self.longi) + "&z=10&t=s")
		if bing:
			pins.append("https://bing.com/maps/default.aspx?cp=" + str(self.lati) + "~" + str(self.longi))
...
class VaxCenter:
	...
	def import_sessions(self, sessions):
		for each_session in sessions:
			...
			self.session_data_sig[vax_session.session_id] = len(self.session_data)-1
...
class VaxFactory:
	...
	def add_center(id, name, addr, block, pincode, 
		...
		min_age_limit, vax, cap_avail, 
		cap_avail_dose1, cap_avail_dose2, slots):
		vax_center = VaxFactory.find_or_create(id, name, addr, block, pincode, lati, longi)
	...
	def data_import(data, is_json = True):
		...
		for each_center_id in data.keys():
			...
			for each_session in vax_center.session_data:
				VaxFactory.add_age_pool(each_session.min_age_limit, vax_center, each_session)
				VaxFactory.add_vax_pool(each_session.vax, vax_center, each_session)
