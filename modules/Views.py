...
class Views:
    ...
    def export_availabilities_external(self, availabilities):
        ...
        for each_exportable_availability in exportable_data:
            ...
            msg += "\n" + "pincode: " + str(each_exportable_availability["pincode"])
            msg += "\n" + "Vax: " + str(each_exportable_availability["vax"])
            msg += "\n" + "Age: " + str(each_exportable_availability["min_age"])
            ...
            if Views.ENABLE_PRECISE_LOCATION:
                ...
                if len(locations) > 0:
                    msg += "\n Locations:"
                    msg += "\n -- <a href='%s'>Google Maps</a>"%(locations[0])
            ...
            for each_slot in slots:
                msg += "\n" + " - Slot time: " + str(each_slot)
            msg += "\n -------------- "

            batch_iteration += 1
            if batch_iteration <= Views.BATCH_EXPORT_LIMIT:
                if len(msgs) == 0:
                    msgs.append(Emojis.vax + " Vax availability detected in accordance to your preferences:" + "\n")
                msgs[-1] += msg
            else:
                msgs.append(Emojis.vax + " Vax availability detected in accordance to your preferences:" + "\n")
                msgs[-1] += msg
